...
class MyStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def _init_custom_modules(self, cfg):
        ...
        self.use_lora = cfg["unet_lora"]["use_lora"]
        if self.use_lora:
            self.lora_rank = cfg["unet_lora"]["rank"]
        ...
        # pipe.id_encoder_adapter # (B, 512) -> (B, 2048)
        # pipe.clip_img_encoder # (B, 3, 224, 224) -> (B, 768)
        # pipe.id_clip_encoder_adapter # (B, 768) -> (B, 2048)
        # pipe.hair_encoder # (B, 3, 512, 512) -> (B, 18, 512)
        # pipe.hair_encoder_adapter_1 # (B, 18, 512) -> (B, 512)
        # pipe.hair_encoder_adapter_2 # (B, 512) -> (B, 2048)
        # pipe.hair_clip_encoder_adapter # (B, 768) -> (B, 2048)
    ...
    def save_adapters(self, path):
        ...
        if self.use_lora:
            lora_state_dict = {k: v for k, v in self.unet.state_dict().items() if "lora" in k.lower()}
            adapters["unet_lora"] = lora_state_dict
    ...
    def load_adapters(self, path, strict=True):
        ...
        if self.use_lora:
            self.unet.load_state_dict(adapters["unet_lora"], strict=False)

    def get_unet_lora_params(self):
        for name, param in self.unet.named_parameters():
            if "lora" in name.lower():
                yield param

    def trainable_parameters(self):
        unet_lora_params = list(self.get_unet_lora_params())
        base_params = itertools.chain(*[module.parameters() for module in self.trainable_modules])
        return itertools.chain(base_params, unet_lora_params)
    ...
    def set_eval_mode(self):
        for module in (self.trainable_modules + self.non_trainable_modules):
            module.eval()
        # No need to do any changes for the self.use_lora = True case, as lora layers behave same in both train/eval modes
    ...
    def print_param_info(self):
        ...
        unet_lora_params = sum(p.numel() for p in self.get_unet_lora_params())
        ...
        print(f"unet_lora_params: {unet_lora_params:,}")
    ...
    def updated_encode_prompt(
        ...
        if update_prompt_embeddings:
            ...
            hairstyle_idxs, gender_idxs = self.get_token_positions(prompt)
    ...
    def get_token_positions(self, prompt):
        man_list = ["man", " man", "man's"]
        woman_list = ["woman", " woman", "woman's"]
        hairstyle_list = ["hairstyle"]

        assert all(any(g in p for g in (man_list + woman_list)) for p in prompt), "Each prompt must contain 'man' or 'woman'"
        assert all(any(h in p for h in hairstyle_list) for p in prompt), "Each prompt must contain 'hairstyle'"

        man_token_ids = [self.tokenizer(m).input_ids[1:-1][0] for m in man_list]
        woman_token_ids = [self.tokenizer(w).input_ids[1:-1][0] for w in woman_list]
        hairstyle_token_ids = [self.tokenizer(h).input_ids[1:-1][0] for h in hairstyle_list]
        ...
        def find_first_occurrence(batch_ids, candidate_ids):
            batch_size, seq_len = batch_ids.shape
            all_idxs = []
            for i in range(batch_size):
                idx = seq_len  # default "not found"
                for cand in candidate_ids:
                    mask = (batch_ids[i] == cand)
                    if mask.any():
                        first_idx = mask.nonzero(as_tuple=True)[0][0].item()
                        if first_idx < idx:
                            idx = first_idx
                all_idxs.append(idx)
            return torch.tensor(all_idxs)

        hairstyle_idxs = find_first_occurrence(prompt_tokens_ids, hairstyle_token_ids)

        gender_idxs = []
        for i, p in enumerate(prompt):
            if any(w in p for w in woman_list):
                ids = woman_token_ids
            else:
                ids = man_token_ids
            gender_idxs.append(find_first_occurrence(prompt_tokens_ids[i].unsqueeze(0), ids)[0].item())
        gender_idxs = torch.tensor(gender_idxs)

        for i, p in enumerate(prompt):
            if hairstyle_idxs[i].item() == self.tokenizer.model_max_length:
                print(f"⚠️ Hairstyle token not found in prompt (possibly truncated or not present): {p}")
                hairstyle_idxs[i] = 0
            if gender_idxs[i].item() == self.tokenizer.model_max_length:
                print(f"⚠️ Gender token not found in prompt (possibly truncated or not present): {p}")
                gender_idxs[i] = 0

        return hairstyle_idxs, gender_idxs
    ...
    def sample(
            ...
            inverted_latents: Optional[torch.Tensor]=None,
        ...
        prompt_embeds, added_cond_kwargs = self.get_text_embeddings(
        ...
        head_noise = torch.randn_like(latents)
        if initial_latent is not None:
            alpha_t = self.scheduler.alphas_cumprod[start_step]
            added_noise = torch.sqrt(alpha_t) * initial_latent + torch.sqrt(1 - alpha_t) * head_noise
        else:
            added_noise = head_noise
        # latents = latents * (1 - io_mask) + added_noise * (io_mask)
        ...
        for i in tqdm(range(start_step, num_inference_steps), disable=not show_progress):
            ...
            if apply_io_mask:
                ...
                if inverted_latents is not None:
                    latents = inverted_latents[:, num_inference_steps -1 - i, :, :, :]
    ...
    def invert(
        ...
        latents = start_latents.clone()
        intermediate_latents = [latents.unsqueeze(1)]
        latent_noises = []
        ...
        for i in tqdm(range(0, invert_till), disable = not show_progress):
            ...
            intermediate_latents.append(latents.unsqueeze(1))
            latent_noises.append(noise_pred.unsqueeze(1))

        latents = torch.concat(intermediate_latents, dim = 1)
        latent_noises = torch.concat(latent_noises, dim = 1)
        return latents, latent_noises
    ...
    def forward(
        ...
        head_mask = torch.tensor(head_mask, dtype = self.dtype, device = self.device).unsqueeze(1)
        with torch.no_grad():
            latents, latent_noises = self.invert(
                images = body_images,
        ...
        B = latents.shape[0]
        rand_timesteps = torch.randint(low = 0, high = num_inference_steps - invert_till, size = (B,))
        t = torch.as_tensor(self.scheduler.timesteps[rand_timesteps]) # , device = self.scheduler.timesteps.device
        latent = latents[torch.arange(B), rand_timesteps, :, :, :] # 0, 1, 2, 3, ...t latent (t = invert_till)
        alpha_t = self.scheduler.alphas_cumprod[t.cpu()].to(latents.device)
        head_noise = torch.randn_like(latent)
        added_noise = torch.sqrt(alpha_t) * latents[:, 0, :, :, :] + torch.sqrt(1 - alpha_t) * head_noise

        latent = latent * (1 - head_mask) + added_noise * (head_mask)
        ...
        head_mask_loss = F.mse_loss(io_masks, head_mask.squeeze(1))
        noise_pred_loss = F.mse_loss(noise_pred * (head_mask), head_noise * (head_mask)) + F.mse_loss(noise_pred * (1 - head_mask), latent_noises[torch.arange(B), rand_timesteps - 1, :, :, :] * (1 - head_mask))
        return head_mask_loss, noise_pred_loss
    ...
    def predict(
            ...
            _io_masks: Optional[torch.Tensor] = None,
            output_type: str = "pil_image",
            invert_till: int = 40,
            num_inference_steps: int = 50,
            show_progress: bool = True
        ...
        if _io_masks is None:
            _io_masks = (io_masks >= io_mask_threshold).float().to(dtype = self.dtype, device = self.device)
        else:
            _io_masks = torch.tensor(_io_masks).to(dtype = self.dtype, device = self.device)
        start_step = num_inference_steps - invert_till
        output_images = self.sample(
            prompt = body_image_captions,
            start_step = start_step,
            start_latents = latents[:, -1, :, :, :],
            initial_latent = latents[:, 0, :, :, :],
            inverted_latents = None, # latents,
            num_inference_steps = num_inference_steps,
            update_prompt_embeddings = True,
            id_images = head_images,
            hair_images = self.apply_masks(head_images, head_images_hair_mask),
